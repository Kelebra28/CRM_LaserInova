  ...
  return (
    <div className="space-y-8 max-w-7xl pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div className={`p-6 rounded-2xl border shadow-sm ${grossProfit >= 0 ? 'bg-gray-900 text-white' : 'bg-red-600 text-white'}`}>
          <div className="flex items-center gap-3 mb-4">
            <div className={`p-2 rounded-lg ${grossProfit >= 0 ? 'bg-white/10 text-emerald-400' : 'bg-white/20 text-white'}`}>
              <TrendingUp className="h-4 w-4" />
            </div>
            <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">Utilidad Real (Proyectos)</span>
          </div>
          <p className="text-2xl font-black">${grossProfit.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</p>
          <p className="text-[9px] font-bold text-white/40 uppercase mt-1">Ganancia neta de tus trabajos</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm border-t-4 border-t-red-500">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Balance Final (Caja)</span>
          </div>
          <p className="text-2xl font-black text-gray-900">${netProfit.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</p>
          <p className="text-[9px] font-bold text-gray-400 uppercase mt-1">Restando Luz, Gasolina, etc.</p>
        </div>
